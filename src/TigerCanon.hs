...
fixBinops :: [Stm] -> [Stm]
fixBinops ((Move (Temp t1) e1):(Move (Temp t2) e2):(Move (Temp t3) (Binop oper e1' e2')):rest) 
  | t1 /= t2 && e1 == e1' && e2 == e2' = (Move (Temp t1) e1):(Move (Temp t2) e2):(Move (Temp t3) (Binop oper (Temp t1) (Temp t2))):rest
  | t1 /= t2 && e1 == e2' && e2 == e1' = (Move (Temp t1) e1):(Move (Temp t2) e2):(Move (Temp t3) (Binop oper (Temp t2) (Temp t1))):rest
  | otherwise = (Move (Temp t1) e1):(Move (Temp t2) e2):(Move (Temp t3) (Binop oper e1' e2')):rest
fixBinops (x:xs) = x:fixBinops xs
fixBinops xs = xs
